...
std::string		noticeResponse(Server &server, int index, std::string &destination, std::string &reason) {
	std::string response;

	response += ":" + server.getNick(index) + "!ircserv NOTICE " + destination + " " + reason + "\n";
	return (response);
}
